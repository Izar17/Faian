...
<div class="row">
					...
					<table class="table" id="revenueTable">
						...
						<tbody>
							<tr>
								<th>Total</th>
								<td>
									<?php echo 'P' . number_format($cashToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($eWalletToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($bankToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($ccToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($totalRevenueNoExpense, 2) . ''; ?>
								</td>
							</tr>
							<tr>
								<th>Expenses</th>
								<td>
									<?php echo 'P' . number_format($cashExpenseToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($gcashExpenseToday, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($bankExpenseToday, 2) . ''; ?>
								</td>
								<td>-</td>
								<td>
									<?php echo 'P' . number_format($expensesToday, 2) . ''; ?>
								</td>
							</tr>
							<tr>
								<th>Grand Total</th>
								<td>
									<?php echo 'P' . number_format($cashGT, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($gcashGT, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($bankGT, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($ccToday, 2) . ''; ?>
								</td>
								<th>
									<?php echo 'P' . number_format($totalRevenue, 2) . ''; ?>
								</th>
							</tr>
						</tbody>
					</table>
					...
					<table class="table" id="productTable">
						...
						<tbody>
							<tr>
								...
								<td>
									<?php echo '' . $layawayRS . 'g'; ?>
								</td>
								<td>
									<?php echo '' . $reservationRS . 'g'; ?>
								</td>
								<td>
									<?php echo '' . $pickupRS . 'g'; ?>
								</td>
								<td>
									<?php echo '' . $deliveryRS . 'g'; ?>
								</td>
								<td><strong>
										<?php echo '' . $Rqty . 'pc'; ?>
									</strong></td>
	...
	<?php } else { ?>
		...
		<div class="col-md-8">
			<div class="panel panel-default">
				<div class="panel-heading"> <i class="fa fa-money"></i> Expenses |
					<?php echo $curDate; ?>
					<div class="pull-right">

						<?php if ($resultShift->num_rows == 1) { ?>
							<a href="expenses.php"><button><i class="glyphicon glyphicon-plus-sign"></i>Add
									Expenses</button></a>
						<?php } else { ?>
							<button data-toggle="modal" data-target="#addEODModel"><i class="glyphicon glyphicon-plus-sign"></i>
								Start Shift</button>
						<?php } ?>
					</div>
				</div> <!-- /div-action -->
					...
					<table class="table" id="revenueTable">
						...
						<tbody>
							<tr>
								<th>Total</th>
								<td>
									<?php echo 'P' . number_format($actRevCash, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($actRevEwallet, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($actRevBank, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($actRevCc, 2) . ''; ?>
								</td>
								<td>
									<?php echo 'P' . number_format($totalRevenueTodayNoExpense, 2) . ''; ?>
								</td>
							</tr>
							<tr>
								<th>Expenses</th>
							...
							<tr>
								...
								<th>Grand Total</th>
								<th>
									<?php echo 'P' . number_format($totalRevenueToday, 2) . ''; ?>
								</th>
							</tr>
						</tbody>
					</table>
		...
		<div class="modal-content">
			...
			<form class="form-horizontal" id="submitEODForm" action="php_action/createEODRevenue.php" method="POST">
				<div class="modal-header">
					...
					<h4 class="modal-title">Your the cashier now?</h4>
				</div>
				<div class="modal-body">
					...
					<div class="form-group" style="text-align:center">
						...
						<button type="submit" class="btn btn-primary" id="createEODBtn" data-loading-text="Loading..."
							autocomplete="off">Yes</button>
						<input type="hidden" name="userId" value="<?php echo $_SESSION['userId']; ?>" />
						<input type="hidden" step="0.01" value="<?php echo $actRevCash; ?>" class="form-control"
							id="actualCash" placeholder="Actual Cash Amount" name="actualCash" autocomplete="off"
							required />

						<input type="hidden" step="0.01" value="<?php echo $actRevEwallet; ?>" class="form-control"
							id="actualEwallet" placeholder="Actual E-Wallet Amount" name="actualEwallet"
							autocomplete="off" required />

						<input type="hidden" step="0.01" value="<?php echo $actRevBank; ?>" class="form-control"
							id="actualBank" placeholder="Actual Bank Amount" name="actualBank" autocomplete="off"
							required />

						<input type="hidden" step="0.01" value="<?php echo $actRevCc; ?>" class="form-control"
							id="actualCc" placeholder="Actual Credit Card Amount" name="actualCc" autocomplete="off"
							required />
					</div>
				</div> <!-- /form-group-->
		</div> <!-- /modal-body -->
